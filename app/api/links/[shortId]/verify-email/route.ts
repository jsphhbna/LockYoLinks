...
export async function POST(request, { params }) {
  try {
    ...
    // Verify email
    if (!linkData.isInviteOnly) {
      return NextResponse.json({ error: "This link is not invite-only" }, { status: 400 })
    }

    const allowedEmails = linkData.allowedEmails || []
    if (!allowedEmails.includes(email)) {
      return NextResponse.json({ error: "This email is not authorized to access this link" }, { status: 401 })
    }
